			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.weixiuyuanName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
						...
						<view v-if="commentList.length>0" v-for="(item,index) in commentList" class=""
							...
							<view v-bind:key="index" class="" style="align-items: center;">
									...
									<view class="" style="font-size: 14px;display: flex;flex-direction: column;color: #8799A3;"
										...
										<view class="">
											<uni-rate size="13" style="float: left;" allow-half readonly
													  :value="item.weixiuyuanCommentbackPingfenNumber" />
										</view>
										<!-- 时间 -->
								...
								<view style="width: 80%;margin-left: 100rpx;">
									<!-- 评价 -->
									{{item.weixiuyuanCommentbackText}}
								</view>
							</view>
							<!-- 回复 -->
		...
		export default {
			...
			async onShow(options) {
				...
				let data = {
					...
					average: 'weixiuyuan_commentback_pingfen_number',
					condition1: 'weixiuyuan_id',
					condition1Value: this.weixiuyuanId,
				}
			...
			methods: {
                ...
                async submitData() {
                    ...
                    await this.$api.save(`weixiuyuanLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('weixiuyuan', this.weixiuyuanId);
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('weixiuyuanCommentback', {
						page: mescroll.num,
						limit: mescroll.size,
						weixiuyuanId: this.weixiuyuanId
					});
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('weixiuyuanYuyue', {
									yonghuId: _this.user.id,
									weixiuyuanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/weixiuyuanYuyue/list');
							}
						}
					});
				},
