	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.guzhangshangbaoId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('guzhangshangbao', this.guzhangshangbaoId);
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('guzhangshangbaoYuyue', {
									yonghuId: _this.user.id,
									guzhangshangbaoId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/guzhangshangbaoYuyue/list');
							}
						}
					});
				},
