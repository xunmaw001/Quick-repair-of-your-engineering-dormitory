		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input
					...
					v-model="searchForm.weixiuyuanCommentbackText" type="text" placeholder="评价内容"></input>
			</view>
			...
			<view class="action">
				...
				<view v-if="isAuth('weixiuyuanCommentback','新增')" @click="onAddTap()"
					:style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
					新增
				</view>
				<view
			...
			<view class="list b-b" v-for="(item, index) in list" :key="index"
				...
				<view class="wrapper" style="padding: 10rpx 3rpx;margin-left: 25upx;margin-top: 10upx;">
					<view style="font-size: 30rpx;display: flex;justify-content: space-between;">
						<b>{{item.weixiuyuanName}}</b>
						<uni-rate style="float: right" readonly v-model="item.weixiuyuanCommentbackPingfenNumber" />
						<view style="float: right;padding: 10rpx 10rpx;" class="cuIcon-delete"
							@click.stop="onDeleteTap(item.id)"></view>
					</view>
					<view class="address-box" style="padding: 10rpx;">
						<text style="margin-left: 30rpx;">我的评价：{{item.weixiuyuanCommentbackText}}</text>
					</view>
					<view class="u-box" style="color: #8799A3;"
						v-if="item.replyText!= null && item.replyText!= 'null' && item.replyText!= ''">
						<text class="name" style="margin-left: 30rpx;font-size: 23rpx;">回复：{{item.replyText}}</text>
					</view>
					<text
						style="float: right;padding: 10rpx 20rpx 20rpx;font-size: 20rpx;color:#8799A3;">{{item.insertTime}}</text>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				let _this = this
				let res = await this.$api.page(`weixiuyuanCommentback`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('weixiuyuanCommentback', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							data.weixiuyuanCommentbackShenheTime = shijian;
							await _this.$api.update(`weixiuyuanCommentback`, data);
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
			...
			async search() {
				...
				if (this.searchForm.weixiuyuanCommentbackText) {
					searchForm['weixiuyuanCommentbackText'] = this.searchForm.weixiuyuanCommentbackText
				}

				let res = await this.$api.page(`weixiuyuanCommentback`, searchForm);
				let _this = this
				res.data.list.forEach(function(item, index) {})
				// 如果是第一页数据置空
