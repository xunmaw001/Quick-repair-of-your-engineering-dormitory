	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.weixiuyuanCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('weixiuyuanCommentback', this.weixiuyuanCommentbackId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.weixiuyuanCommentbackPhoto ? this.detail.weixiuyuanCommentbackPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('weixiuyuanCommentbackYuyue', {
									yonghuId: _this.user.id,
									weixiuyuanCommentbackId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/weixiuyuanCommentbackYuyue/list');
							}
						}
					});
				},
